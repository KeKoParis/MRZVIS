...
Реализовать модель сети Джордана-Элмана с недовыпрямленной линейной функцией активации (Leaky ReLU)

Выполнил
Войткус С.А. гр.121703

источник: 
https://medium.com/@tommasocaputi85/hopfield-network-python-implementation-05a9f2f337de
"""
...
if __name__ == "__main__":
    ...
    learning_rate = 0.00001  # шаг обучения
    max_gradient_norm = 1.0  # Ограничение градиента
    max_error_threshold = float(input("Enter maximum error threshold for training: "))
    eps = 1e-8
    ...
    while True:
        ...
        # Прерывание при NaN/Inf
        if not np.isfinite(error):
            print("Training stopped due to NaN/Inf values.")
            break

        # Прерывание при достижении порога ошибки
        if error <= max_error_threshold:
            print("Training stopped as error threshold reached.")
            break
